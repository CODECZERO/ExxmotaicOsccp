...
def override_get_db(monkeypatch):
    ...
    monkeypatch.setattr("api.controllers.live_controller.get_db", mock_get_db)
    ...
    monkeypatch.setattr(
        "api.controllers.meter_values_controller.db_available", lambda: True
    )
    monkeypatch.setattr("api.controllers.stats_controller.db_available", lambda: True)
    monkeypatch.setattr("api.controllers.live_controller.db_available", lambda: True)
...
def test_charger_crud(client: FlaskClient):
    ...
    # 1. Create a charger
    payload = {"charger_id": "TEST_CHG_001", "vendor": "TKT", "model": "DC-60kW"}
...
def test_remote_commands(client: FlaskClient):
    ...
    payload = {"charger_id": "TEST_CHG_002", "vendor": "TKT", "model": "AC-22kW"}
    client.post("/api/chargers", json=payload)

    # Send a remote start command
    response = client.post(
        "/api/chargers/TEST_CHG_002/start", json={"id_tag": "USER123"}
    )
    assert response.status_code == 200
    data = response.get_json()
...
def test_live_stream_emits_initial_snapshot(client: FlaskClient):
    """The SSE stream should emit an initial update event immediately."""
    payload = {
        "charger_id": "TEST_CHG_STREAM",
        "vendor": "TKT",
        "model": "DC-60kW",
    }
    create_response = client.post("/api/chargers", json=payload)
    assert create_response.status_code == 201

    response = client.get("/api/live/stream?charger_id=TEST_CHG_STREAM", buffered=False)
    first_chunk = next(response.response)
    response.close()

    assert b"event: update" in first_chunk
    assert b"TEST_CHG_STREAM" in first_chunk
